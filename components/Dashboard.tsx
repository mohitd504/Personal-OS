      ...
      <div className={`main view-${view}`}>
        <div className="topbar">
          ...
          <div className="row" style={{gap:6,flexWrap:"wrap"}}>
            ...
            <span className="build-mark" title="build marker — bump this to verify a deploy went live">build&nbsp;109</span>
          </div>
        </div>
...
function seedAllCourses(start?:Date, overwrite:boolean=true){
  ...
  DSA_COURSE.forEach((day:any,i:number)=>{
    setDay("dsa",i,{ id:uid(), courseId:"dsa", label:`DSA Day ${i+1}: ${day.title}`, hours:"1.5", brief:`${day.brief}  📖 Book: ${day.read}`, resource:DSA_PLAYLIST, pdf:`/course/dsa-day-${String(i+1).padStart(2,"0")}.pdf`, video:`📖 Read from "Data Structures Through C in Depth" — ${day.read}`, courseVideo:DSA_PLAYLIST, plan:day.plan||[], next:[] }); });
}
