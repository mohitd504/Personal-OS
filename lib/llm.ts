...
export async function askLLMImage(system: string, text: string, dataUrl: string, maxTokens = 400): Promise<string> {
  const oai = process.env.OPENAI_API_KEY;
  if (oai) {
    try {
      const r = await fetch("https://api.openai.com/v1/chat/completions", {
  ...
  if (anth) {
    try {
      ...
      const r = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        headers: { "x-api-key": anth, "anthropic-version": "2023-06-01", "content-type": "application/json" },
        body: JSON.stringify({
          model: process.env.ANTHROPIC_MODEL || "claude-haiku-4-5-20251001", max_tokens: maxTokens, system,
          messages: [{ role: "user", content: [{ type: "text", text }, { type: "image", source: { type: "base64", media_type: media, data: b64 } }] }],
        }),
      });
      const d = await r.json();
      return d?.content?.[0]?.text || "";
